...
%  P. Doll?, Z. Tu, P. Perona and S. Belongie
...
if( ~isfield(pChns,'complete') || pChns.complete~=1 || isempty(I) )
  ...
  pChns = getPrmDflt(varargin,{'shrink',4,'pColor',{},'pGradMag',{},...
    'pGradHist',{},'pCustom',p,'complete',1,'s',1},1);
  ...
  pChns.pGradMag = getPrmDflt( pChns.pGradMag, {'enabled',1,...
    'colorChn',0,'normRad',5,'normConst',.005,'full',0,'gmcMethod','','gmcParam',[]}, 1 );
...
% GMC Procedure
if (~isempty(chns.pChns.pGradMag.gmcMethod) && ~isempty(chns.pChns.pGradMag.gmcParam))
     if (pChns.s == 1)
     elseif (pChns.s < 1)
         X = computeKernel(pChns.s, 'Quadratic');
         GMC = X * pChns.pGradMag.gmcParam{1};
         M=M*GMC;
     else
         X = computeKernel(pChns.s, 'Linear');
         GMC = X * pChns.pGradMag.gmcParam{2};
         M=M*GMC;
     end
 end
...
% % compute gradient histgoram channels
% p=pChns.pGradHist; nm='dACF';
% if( p.enabled )
%     dACF = cat(3, chns.data{:});
%     
%     for offset = 2 : 4
%     
%         % 1st D
%         sACF = zeros(size(dACF), 'single');
%         sACF(1:end-offset+1,:,:)=(dACF(1:end-offset+1,:,:)-dACF(offset:end,:,:));
%         chns=addChn(chns,sACF,nm,pChns.pGradHist,0,h,w);
% 
%         % 2nd D
%         sACF = zeros(size(dACF), 'single');
%         sACF(:,1:end-offset+1,:)=(dACF(:,1:end-offset+1,:)-dACF(:,offset:end,:));
...
%         % Diagonal 1 
%         sACF = zeros(size(dACF), 'single');
%         sACF(offset:end,1:end-offset+1,:)=(dACF(offset:end,1:end-offset+1,:)-dACF(1:end-offset+1,offset:end,:));
%         chns=addChn(chns,sACF,nm,pChns.pGradHist,0,h,w);
% 
%         % Diagonal 2
%         sACF = zeros(size(dACF), 'single');
%         sACF(1:end-offset+1,1:end-offset+1,:)=(dACF(1:end-offset+1,1:end-offset+1,:)-dACF(offset:end,offset:end,:));
%         chns=addChn(chns,sACF,nm,pChns.pGradHist,0,h,w);
%     end
% end
